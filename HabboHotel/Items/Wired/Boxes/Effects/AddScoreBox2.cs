    ...
    class AddScoreBox2 : IWiredItem, IWiredCycle
    {
        ...
        private void TeleportUser(Habbo Player)
        {
            ...
            if ((Instance == null || User == null ? false : !User.IsBot))
            {
                ...
                {
                    //dbClient.RunFastQuery("UPDATE users SET puntos_eventos = puntos_eventos + '" + mScore + "' WHERE id = '" + User.UserId + "'");
                    //dbClient.RunFastQuery("UPDATE users SET premio = premio + '1' WHERE id = '" + User.UserId + "'");
                }
