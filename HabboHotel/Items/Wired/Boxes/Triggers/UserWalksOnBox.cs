...
namespace StarBlue.HabboHotel.Items.Wired.Boxes.Triggers
{
    internal class UserWalksOnBox : IWiredItem
    {
        ...
        public bool Execute(params object[] Params)
        {
            ...
            if (!User.isRolling && User.Z != Item.TotalHeight && !Item.Data.IsSeat && Item.Data.InteractionType != InteractionType.BED)
            {
                return false;
            }

            List<Item> ItemsForItem = Instance.GetGameMap().GetCoordinatedItems(new Point(User.X, User.Y));
            foreach (Item item in ItemsForItem)
            {
                if (!SetItems.ContainsKey(item.Id) && (item.Data.IsSeat || item.Data.InteractionType == InteractionType.BED))
                {
                    return false;
                }
            }
