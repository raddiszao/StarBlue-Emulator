...
namespace StarBlue.HabboHotel.Moderation
{
    ...
    {
        private static ILog log = LogManager.GetLogger("StarBlue.HabboHotel.Moderation.ModerationManager");
            ...
            using (IQueryAdapter dbClient = StarBlueServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetBans != null)
                {
                    foreach (DataRow dRow in GetBans.Rows)
                    {
                        ...
                        if (Ban != null)
                        {
                            if (expires > StarBlueServer.GetUnixTimestamp())
                            {
        ...
        {
            if (StarBlueServer.GetGame().GetModerationManager().IsBanned(MachineId, out ModerationBan MachineBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = StarBlueServer.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        StarBlueServer.GetGame().GetModerationManager().RemoveBan(MachineId);
                        return false;
                    }
                }
            }
        ...
        {
            if (StarBlueServer.GetGame().GetModerationManager().IsBanned(Username, out ModerationBan UsernameBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = StarBlueServer.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        StarBlueServer.GetGame().GetModerationManager().RemoveBan(Username);
                        return false;
                    }
                }
            }
