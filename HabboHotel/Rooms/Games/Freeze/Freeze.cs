...
using StarBlue.Communication.Packets.Outgoing.Rooms.Freeze;
...
namespace StarBlue.HabboHotel.Rooms.Games.Freeze
{
    public class Freeze
    {
        private Room _room;
        ...
        {
            return ExitTeleports.Values.ToList()[StarBlueServer.GetRandomNumber(0, ExitTeleports.Count - 1)];
        }
        ...
        public void StopGame(bool userTriggered = false)
        {
            ...
            foreach (RoomUser User in this._room.GetRoomUserManager().GetUserList().ToList())
            {
                ...
                if (User.Team == Winners)
                {
                    ...
                    StarBlueServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezeWinner", 1);
        ...
        private void CountTeamPoints()
        {
            ...
            foreach (RoomUser User in this._room.GetRoomUserManager().GetUserList().ToList())
            {
                ...
                //_room.GetGameManager().AddPointToTeam(User.Team, 0);
                User.GetClient().SendMessage(new UpdateFreezeLivesComposer(User.InternalRoomID, User.FreezeLives));
                StarBlueServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezePlayer", 1);
            }
        }
                ...
                case FreezePowerUp.HEART:
                    {
                        //if (User.FreezeLives < 5)
                        {
                            ...
                            StarBlueServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezePowerUp", 1);
                        }
        ...
        private void FreezeUser(RoomUser User)
        {
            ...
            _room.GetGameManager().AddPointToTeam(User.Team, -10);
            User.ApplyEffect(12);
            StarBlueServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezeQuestCompleted", 1);
            //_room.GetGameMap().TeleportToItem(User, GetRandomExitTile());
