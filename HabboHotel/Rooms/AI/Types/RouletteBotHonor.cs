    ...
    class RouletteBotHonor : BotAI
    {
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            if (User.GetClient().GetHabbo().Duckets >= 2 && BetsOpen == true)
            {
                ...
                {
                    GetRoomUser().Chat("Você deve apostar entre 0 e 36, não pode deixar esses limites.", false, 33);
                    return;
                }
                ...
                User.GetClient().SendMessage(RoomNotificationComposer.SendBubble("eventoxx", User.GetClient().GetHabbo().Username + ", acaba de apostar " + IntMultiplier + "  " + StarBlueServer.GetSettingsManager().TryGetValue("seasonal.currency.name") + " a " + IntBet + ".\n\nBoa sorte.", "catalog/open/habbiween"));

                GetRoomUser().Chat("Aposta de " + IntMultiplier + "  " + StarBlueServer.GetSettingsManager().TryGetValue("seasonal.currency.name") + " realizada a " + IntBet + " por " + User.GetClient().GetHabbo().Username + ".", false, 33);
        ...
        public override void OnTimerTick()
        {
            ...
            if (GameLength <= 0)
            {
                ...
                foreach (KeyValuePair<int, CasinoDataLocura> pair in Data1)
                {
                    ...
                    if (Data2.bet == num)
                    {
                        GameClient client = StarBlueServer.GetGame().GetClientManager().GetClientByUserID(Data2.userId);
