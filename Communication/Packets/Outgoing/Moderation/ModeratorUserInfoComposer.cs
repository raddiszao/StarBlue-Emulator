...
namespace StarBlue.Communication.Packets.Outgoing.Moderation
{
    internal class ModeratorUserInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserInfoMessageComposer)
        {
            ...
            base.WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((StarBlueServer.GetUnixTimestamp() - Convert.ToDouble(User["account_created"])) / 60)) : 0);
            base.WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((StarBlueServer.GetUnixTimestamp() - Convert.ToDouble(User["last_online"])) / 60)) : 0);
            base.WriteBoolean(User != null ? StarBlueServer.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(User["id"])) != null : false);
